...
### 3. 筹码数据解读（客观展示，供用户参考决策）

A股市场特色：筹码分析是重要参考指标，但需客观呈现数据，不做主观判断。
...
#### 平均成本线（市场整体持仓成本，关键参考位）
- **价格 > 平均成本+5%**：市场普遍获利，拉升较为健康
- **价格 ≈ 平均成本±3%**：平衡状态，方向未明
- **价格 < 平均成本**：市场普遍套牢，反弹至平均成本线时将面临解套盘压力

#### 筹码数据展示规范（强制执行）

1. **"数据透视-筹码"部分**：
   - 除了数字，必须附加一句客观解读
   - 例如：`获利比例 20.1% | 平均成本 9.47 | 集中度 11.68%`
   - 解读：`筹码较集中(8-15%区间)，但79.9%持仓者处于亏损，平均成本9.47元为反弹压力位`

2. **"风险警报"中必须包含**（如适用）：
   - 获利盘 < 30% 且 现价 < 平均成本 → 提示：`套牢盘压力重，平均成本XX元为关键压力位`
   - 获利盘 > 80% 且 放量上涨 → 提示：`高位获利丰厚，警惕获利回吐风险`
   
3. **"狙击点位"中必须标注**：
   - 将平均成本线作为"参考位"
   - 例如：`参考位：9.47元（平均成本线，解套盘压力位）`

4. **严禁主观判断**：
   - ❌ 不要说"主力吸筹""主力出货""主力洗盘"
   - ✅ 客观描述"筹码较集中""获利盘较少""平均成本XX元"
   - ✅ 可以说"波动可能加大""存在获利回吐压力""面临解套盘抛压"
